...
object ATH {
  def main(args: Array[String]) {
    ...
    var wfmu_dist = wfmu.toDF("ip_address", "client_", "user_", "dt", "channel", "stat", "byt", "ref", "agnt", "secnd").distinct()
    var wfmudf = wfmu_dist.withColumn("mnth", month(from_unixtime(unix_timestamp($"dt", "dd/MMM/yyyy:HH:mm:ss") - $"secnd"))).withColumn("yr", year(from_unixtime(unix_timestamp($"dt", "dd/MMM/yyyy:HH:mm:ss") - $"secnd"))).select("ip_address", "yr", "mnth", "channel", "secnd")
    ...
    wfmu_rdd.coalesce(1,true).map(x=>x.mkString(",")).saveAsTextFile(args(1))
  }
}
